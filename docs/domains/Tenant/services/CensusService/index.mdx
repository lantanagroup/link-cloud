...
The Census service consumes `PatientListsAcquired` and 'CernerPatientsAcquired' events from Kafka. When a new event is received, the Kafka listeners process the payload, updating census records for the specified facility. This enables downstream reporting and data acquisition.

**Workflow Steps:**
1. **Scheduling:** A scheduled job (per tenant configuration) triggers a request for updated patient lists.
2. **Data Acquisition:** The system sends a `DataAcquisitionRequested` event, prompting the acquisition of FHIR patient lists or via SFTP.
3. **Event Production:** Upon successful acquisition, a `PatientListsAcquired` or `CernerPatientsAcquired` event is produced, containing the list of patients.
...
The data model for the Census service will be broken into two main types:

**Event Store:** The database table responsible for exclusively inserting any relevant census information that’s streamed from external sources (ADT, FHIR List, SFTP).  No updates or deletes should be performed on the records written to this table. The Event Store is considered the source of truth for patient census information. Each event store record must provide the source of the event (ADT, SFTP, etc) and include a payload of data related to the event type.

**Materialized View:** A series of tables that represent the current state of a patient’s visit. As records are written to the Event Store, the Census service will update these tables to reflect any changes that were made for the patient. Materialized views are not considered the source of truth.
...
| patientId             | The unique FHIR ID of the patient being discharged.              |
| admitDate             | The datetime when the patient was admitted.                      |
| encounterId           | The FHIR Encounter.Id of the patient.                            |
| finNumber             | The Cerner finNuber assoicated with the patient.                 |
| medicalRecordNumber   | The medical record number of the patient.                        | 
| encounterStatus       | The FHIR Encounter.Status of the patient.                        |
| encounterType         | The FHIR Encounter.Type of the patient.                          |
| payloadType           | Automatically set to "CernerListAdmit" (not included in JSON).   |

This event is used to track patient admissions for Cerner facilities acquired via SFTP. When processed, it creates a new PatientEncounter record in the system with the encounter information from the SFTP entries.

### CernerListDischarge Event
...
| patientId             | The unique FHIR ID of the patient being discharged.                |
| dischargeDate         | The datetime when the patient was discharged.                      |
| payloadType           | Automatically set to "CernerListDischarge" (not included in JSON). |

This event is used to track patient discharges for Cerner facilities when a patient is no longer found on the latest SFTP acquisition. When processed, it updates the existing patient PatientEncounter record in the system with the a discharge date.

### CernerListUpdate Event
...
| encounterStatus       | The FHIR Encounter.Status of the patient.                        |
| encounterType         | The FHIR Encounter.Type of the patient.                          |
| payloadType           | Automatically set to "CernerListUpdate" (not included in JSON).  |

This event is used to track Patient admission updates for Cerner facilities acquired via SFTP. When processed, it updates the existing PatientEncounter record in the system with updated information from the consumed SFTP entries.
