...
The Normalization service provides comprehensive testing capabilities to validate operations before applying them to production data.

**Features:**
- Real-time testing with immediate results
- JSON input validation with error reporting
- Resource type validation against operation configuration
- Transformed output display with original value preservation

**API Endpoints:**
- `POST /api/Normalization/Operations/test` - Test a new operation configuration
- `POST /api/Normalization/Operations/{operationId}/test` - Test an existing operation
...
**Testing Workflow**:
1. Select appropriate FHIR resource type
2. Input sample FHIR resource JSON data
3. Execute test to validate transformation logic
4. Review transformed output with preserved original values
...
- **Comprehensive testing capabilities** allow validation of operations before production deployment.
...
<AccordionGroup>
  <Accordion title="Test Individual Operations">
    Test any normalization operation against sample FHIR resources to validate transformation logic.
    ...
    **Features**:
    ...
    - Resource type validation against operation configuration
    - Transformed output display with original value preservation
    - Support for both existing and new operation configurations
    
    **API Endpoints**:
    - `POST /api/Normalization/Operations/test` - Test a new operation configuration
    - `POST /api/Normalization/Operations/{operationId}/test` - Test an existing operation
    
    **Request Format** (Test New Operation):
    ```json
    {
      ...
      "operation": {
        ...
        "resourceType": "Patient",
        "fhirPath": "Patient.extension.where(url = 'http://hl7.org/fhir/us/core/StructureDefinition/us-core-race').extension.where(url = 'ombCategory').value.code",
        "conceptMaps": [
          {
            "source": { "code": "2054-5", "system": "urn:oid:2.16.840.1.113883.6.238" },
            "target": { "code": "B", "system": "http://terminology.hl7.org/CodeSystem/v3-Race" }
          }
        ]
      },
    ...
    **Request Format** (Test Existing Operation):
    ```json
    {
      "resourceType": "Patient",
      "id": "example-patient",
    ...
    **Response Format**:
    ```json
    {
      ...
      "resource": {
        ...
        "extension": [
          {
            "url": "http://link.extension.org/original-value",
            "valueString": "original-code-value"
          }
        ]
      }
    }
...
- **Testing capabilities** enable validation of operation configurations before deployment, reducing the risk of production errors.