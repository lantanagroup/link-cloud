...
As of 8/21/2025, the Link team is gathering requirements to present to Leidos on the HSLOC and Org Id fields facilities will need to maintain through the NHSN app. Below are some initial ideas on what Link would need from an NHSN hosted API to access those mappings:
...
**Org Id Resolution**: If Data Acquisition determines that a patient does not have any Encounters that map to the reporting organization, an event should be published to the Census service so that the service can clear census records for that patient.
