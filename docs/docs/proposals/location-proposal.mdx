...
stateDiagram-v2
    ...
    exception_org_id: Remove Acquired Resources and Patient of Interest Record
    exception_hsloc: Won't Evaluate Patient
...
**Org Id Resolution**: If Data Acquistion determines that a patient does not have any Encounters that map to a reporting organization, an event should be published to the Census service so that the service can clear patient of interest records for that patient.
...
- The FHIR Location Id
- The HSLOC code
- Ignore (Un-mapped location that does not need to mapping from external endpoint)
