...
Link Cloud currently does not have relationship hierarchy present that ties a reporting facility to their parent hospital network. The Link team has identified that hospital networks often share a single FHIR endpoint as its data source. Because of this, some facility configurations can be redundant. Additionally, there may be query and reporting efficiencies that we can access through this newly present hierarchy. This proposal will target developing the tenant/facility hierarchy across each Link Cloud service. Data models, Kafka events and service API updates will need to be made to support the requirements detailed in this proposal.

## Current Configuration Implementation

The current implementation of Link Cloud represents each facility record as a tenant. Because of this, each tenant that enrolls in the system is at the facility level. Below is a diagram of the current configuration relationship for each facility:
...
erDiagram
	...
	CENSUS_EVENT}o--o{FHIR_LIST:"0..* sourced from 0..*"
	CENSUS_EVENT}o--o{SFTP_FILE:"0..* sourced from 0..*"
...
In our current implementation, a health system **must** configure each facility separately regardless if they may share similar settings. For example, if a Tenant has a single FHIR server data source, each facility will still need to apply the same Data Acquisition configs per facility. This is an unnecessary burden on sites that decide to enroll into Link Cloud. Other types of duplicate configuration burdens (Census scheduling, Acquisition Lag, etc.) can exist when a tenant also configures multiple facilities. 
...
| Link services must be capable of configuring a single tenant/multi-facility relationship hierarchy | Interface |
| Link Admin UI must be capable of configuring a single tenant/multi-facility relationship hierarchy | Interface |
...
| The Data Acquisition service must maintain and schedule census acquisition configurations for tenants and facilities | System | 

## Proposed Change Summary

### Single Tenant Multi-Facility Relationship

The new proposed relationship will be built where the managing tenant (health system) has one-to-many reporting facilities. Like Link Cloud's current design, each facility will be responsible for their own reporting. Adding a tenant relationship to facilities allows for sites to configure on either a tenant level (encompassing all tenant facilities) or the facility level (for facilities that need targeted configurations that differ from the parent tenant). For example, a tenant can configure a single FHIR server that all child facilities will interface with for reporting. But if one of the facilities for a tenant uses a different data source, they can still have control over configuring for their site. To support this change, services across Link Cloud will need their API's, Kafka Consumers and Producers, and Data Models updated. Below is the proposed relationship with tenants and facilities to various entities in Link Cloud:
...
erDiagram
	...
	CENSUS_EVENT}o--o{FHIR_LIST:"0..* sourced from 0..*"
	CENSUS_EVENT}o--o{SFTP_FILE:"0..* sourced from 0..*"
	CENSUS_EVENT}o--o{ADT_EVENT:"0..* sourced from 0..*"
...
**Normalizations**: Until we observe that it is necessary, Normalization operations will still be managed per facility.
...
**Acquisition and Census Data Source**: The Data Acquisition data sources (FHIR Patient List config, FHIR Acquisition config, SFTP census config) should be capable of being managed at the tenant level. If a facility wants to configure a data source at the facility level, they should be able to do so. 

### Census Acquisition Scheduling Change

With Census acquisition scheduling being configured at the tenant or overridden at the facility level, it's proposed that the schedule configuration management that determines the frequency for acquiring a new FHIR List or SFTP file be done in Data Acquisition rather than the Census service. By moving the scheduling configuration to Data Acquisition, the scheduled configurations can closely align with the acquisition data source configurations made for tenants and their facilities. For example, it would be easier for Data Acquisition to manage both the scheduling and acquisition for a tenant that has a single file in an SFTP server for all facilities. Rather than having Census coordinate with how the acquisitions are configured to determine how many `PatientCensusScheduled` events need to be generated, the Data Acquisition service can own that responsibility. Below are the current and proposed workflows for census acquisition:

#### Current Census Acquisition Sequence

```mermaid
sequenceDiagram
	autonumber
	User->>Census: Creates acquisition scheduling
	Census->>Quartz: Create schedule
	Quartz->>Census: Scheduled duration met
	Census->>Kafka: Produces PatientCensusScheduled event for facility
	Kafka->>Data Acquisition: Consumes PatientCensusScheduled
	par Patient List Acquisition
		Data Acquisition->>Epic EHR: Acquire Patient List
		Data Acquisition->>Kafka: Produces PatientListsAcquired event
		Kafka->>Census: Consumes PatientListsAcquired event			
	and Cerner SFTP Acquisition
		Data Acquisition->>SFTP Server: Acquire File
		Data Acquisition->>Kafka: Produces CernerPatientsAcquired event
		Kafka->>Census: Consumes CernerPatientsAcquired event			
	end
```

#### Proposed Census Acquisition Sequence

```mermaid
sequenceDiagram
	autonumber
	User->>Data Acquisition: Creates acquisition scheduling
	Data Acquisition->>Quartz: Create schedule
	Quartz->>Data Acquisition: Scheduled duration met
	par Patient List Acquisition
		Data Acquisition->>Epic EHR: Acquire Patient List
		Data Acquisition->>Kafka: Produces PatientListsAcquired event
		Kafka->>Census: Consumes PatientListsAcquired event			
...
When tenant related API requests are made by the user or system, the Link service **must** validate that the Tenant exists in the Tenant service. This functionality should be like current service functionality that validates that the tenant exists. Additionally, services have a Service Registry app setting to bypass Tenant validation. A similar app config should be available for Tenant validation bypasses. An example of the current config is below:
...
- Data querying efficiency strategy for tenants will be addressed in a separate proposal.

## Proposed Service Changes

### Tenant Service

#### Data Relationship Updates
The Tenant Service data model **must** maintain a relationship of a single Tenant record to many child Facilities. Each of the child Facility records will be responsible for maintaining their own individual scheduled reports (exactly as it does in Link Cloud's current state):
...
**Endpoint Removal**: `api/census/{facility-id}/history/admitted` should be able to be removed. The patient-encounter API endpoints (current and historical) should be able to return the same data.  

**Config Scheduling**: Quartz scheduling will need to be applied when either a tenant or facility configuration is made. The following logic should be considered when applying a census acquisition schedule:
	- When a census scheduling config is made for a tenant, `PatientCensusScheduled` events should be generated without `facilityIds` being populated.
	- When a census scheduling config is made for a facility, `PatientCensusScheduled` events should be generated exclusively for the facility. The facility **must** no longer be included on the tenant census schedule.   
	- When a census schedule config is deleted for a facility, the facility **must** be included in the tenant census schedule.
...
erDiagram
	...
	TENANT||--|{FACILITY:"1 has 1..*"
	FACILITY||--o|FHIR_CONNECTION:"1 configures 0..1"
	TENANT||--o|CENSUS_ACQUISITION:"1 configures 0..1"
	FACILITY||--o|CENSUS_ACQUISITION:"1 configures 0..1"
...
| DELETE		 	 | /query-plans/tenants/\{tenant-id\}																													| New endpoint: deletes a query plans for a tenant (should validate that all tenant facilities have a query plan before deleting)  |
...
**DataAcquisitionRequested and PatientCensusScheduled Consumer**: When consuming `DataAcquisitionRequested` and `PatientCensusScheduled` events, the Data Acquisition service **must** determine whether the facility for the consumed event contains its own data source configuration or if it uses the tenant configuration. If a facility configuration is available, the Data Acquisition service **must** use it to query for FHIR resources or acquire census information.
...
erDiagram
	TENANT||--|{FACILITY:"1 has 1..*"
	FACILITY||--o{OPERATIONS:"1 configures 0..*"
...
| GET					 | /operations/\{operation-id\} 																				 | Replaces `api/normalization/operations`						 																																|
| DELETE			 | /operations/\{operation-id\}																					 | New endpoint										 										 																																|
| POST				 | /operations/																													 | Replaces `api/normalization/operations`						 																																|
| PUT  				 | /operations/																													 | Replaces `api/normalization/operations`						 																																|
| GET					 | /operations/tenants/\{tenant-id\}/facilities/\{facility-id\} 				 | Replaces `api/normalization/operations/facility/{facility-id}`																											|
| POST				 | /operations/tenants/\{tenant-id\}/facilities/\{facility-id\} 				 | New endpoint										 										 																																|
| PUT  				 | /operations/tenants/\{tenant-id\}/facilities/\{facility-id\} 				 | New endpoint										 										 																																|
| DELETE			 | /operations/tenants/\{tenant-id\}/facilities/\{facility-id\} 				 | Replaces `api/normalization/operations/facility/{facility-id}`																											|
| DELETE			 | /operations/vendors/\{vendor-name\}													 				 | Replaces `api/normalization/operations/vendor/{vendor-name}`  																											|
| POST				 | /operations/$test																										 | Replaces `api/normalization/operations/test`				 																																|
| POST				 | /operations/\{operation-id\}/$test																		 | Replaces `api/normalization/operations/{id}/test`	 																																|
| GET					 | /operation-sequences/tenants/\{tenant-id\}/facilities/\{facility-id\} | Replaces `api/normalization/OperationSequence`			 																																| 
| POST				 | /operation-sequences/tenants/\{tenant-id\}/facilities/\{facility-id\} | Replaces `api/normalization/OperationSequence`			 																																| 
| DELETE			 | /operation-sequences/tenants/\{tenant-id\}/facilities/\{facility-id\} | Replaces `api/normalization/OperationSequence`			 																																| 
...
| ------------ | --------------------------------------------------- 		| ------------------------------------------------------------------------------------------------------ 		|
| GET					 | /info 			                  					  		  	  		|	Replaces `/api/validation/info`  														 																							|
...
**Tenant/Facility Relationship**: Any facility that's created by the user **must** reference to a parent Tenant.

**Tenant Level Configurations**: Tenants can manage some configurations for all of their related facilities. The Admin UI pages that manage configurations **must** support the possibility of a tenant managing the following configurations: 
...
The service requests in the BotW Postman workspace will need to be updated to conform to the new API routes. 
