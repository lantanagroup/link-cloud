...
The Report service is responsible for tracking the reporting progress for each facility. As patients are evaluated across report measures, the Report service will package each patient report(s) into a single aggregate file. After performing the aggregation logic, the files are then sent to the submission destination. When a tenants reporting period end date has been met, the Report Service will initiate evaluations for each patient still admitted within the facility. Once all patient aggregate files are accounted for, the Report service will submit a manifest file to the submission location that contains a summary of the facility reporting period. 
