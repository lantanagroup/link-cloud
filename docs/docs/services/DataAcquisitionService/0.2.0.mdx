...
The Data Acquisition service supports multiple authentication methods to connect to different FHIR servers and EHR systems:

**1. Basic Authentication**
- Uses HTTP Basic Authentication with username and password credentials
- Credentials are Base64-encoded and sent in the `Authorization` header
- Configuration requires `UserName` and `Password` fields
- Suitable for FHIR servers that support basic authentication

**2. Epic Authentication**
- Specifically designed for Epic's FHIR implementation
- Uses OAuth 2.0 client credentials flow with JWT assertion
- Requires Epic-specific configuration including:
  - Client ID
...
- Standard OAuth 2.0 and OpenID Connect authentication
- Supports various OAuth flows depending on the EHR system
- Configuration requires:
  - Client ID and Client Secret
  - Token endpoint
  - Authorization scopes
- Suitable for FHIR servers implementing standard OAuth 2.0

**4. Custom Headers**
- Allows configuration of custom HTTP headers for authentication
- Useful for EHR systems with proprietary authentication mechanisms
- Supports multiple custom headers as key-value pairs
- Headers are added to every FHIR API request
- Configuration requires a `CustomHeaders` dictionary with header names and values
- Does not use query parameters; all authentication data is sent as HTTP headers

Each authentication method is implemented as a separate service implementing the `IAuth` interface, ensuring consistent authentication handling across different EHR systems.
