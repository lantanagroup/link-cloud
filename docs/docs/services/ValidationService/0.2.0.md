...
<AccordionGroup>
  <Accordion title="1. Measure Evaluation Completion">
    - Evaluates data a patient.
    - Persists the patient's report as NDJSON in blob storage.
    - Produces a `MeasureReportGenerated` event for each measure report generated and persisted.
  </Accordion>
  <Accordion title="2. Report Services Persists MeasureReport">
    - The report service consumes `MeasureReportGenerated`.
...
sequenceDiagram
    ...
    MES->>Kafka: Publish "MeasureReportGenerated" message
    Kafka->>VS: Consume "MeasureReportGenerated" message
