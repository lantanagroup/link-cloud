...
export function FacilityHarness() {
  ...
  const [profiles, setProfiles] = useState<TestUserProfile[]>(() =>
    loadProfiles(),
  );
  ...
  function activateProfile(profileId: string) {
    ...
    const nextProfiles = profiles.map((profile) =>
      profile.id === profileId ? { ...profile, lastUsedOn: now } : profile,
    );
  ...
  function removeProfile(profileId: string) {
    ...
    const nextActive =
      activeId === profileId ? (nextProfiles[0]?.id ?? null) : activeId;
  ...
  function handleSubmit(event: FormEvent) {
    ...
    const nextProfiles = editor.id
      ? profiles.map((profile) =>
          profile.id === editor.id ? nextProfile : profile,
        )
    ...
    <div className={`shell${collapsed ? " shell--collapsed" : ""}`}>
      <aside className="shell__sidebar">
        <button
          ...
          aria-label={
            collapsed ? t("shell.expandLabel") : t("shell.collapseLabel")
          }
        ...
        <div className="shell__sidebar-content">
          <h1>{t("app.title")}</h1>
          <p className="shell__tagline">{t("shell.tagline")}</p>
                ...
                {sortedProfiles.map((profile) => (
                  <li key={profile.id}>
                    ...
                    <div className="shell__profile-meta">
                      ...
                      <button
                        type="button"
                        className="shell__button--danger"
          ...
          <div className="shell__card">
            <h2>
              {editor.id
                ? t("shell.updateProfileTitle")
                : t("shell.createProfileTitle")}
            </h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="shell__row">
                ...
                <input
                  ...
                  value={editor.groups}
                  onChange={(event) =>
                    setEditor({ ...editor, groups: event.target.value })
                  }
                  placeholder={t("shell.form.groupsPlaceholder")}
                />
              </div>
              <div className="shell__row">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setEditor({ ...editor, facilityId: event.target.value })
                  }
                  required
                />
              </div>
              <div className="shell__row">
                <label htmlFor="facilityName">
                  {t("shell.form.facilityName")}
                </label>
                <input
                  id="facilityName"
                  value={editor.facilityName}
                  onChange={(event) =>
                    setEditor({ ...editor, facilityName: event.target.value })
                  }
                />
              </div>
              <div className="shell__row">
                ...
                <input
                  ...
                  value={editor.issuer}
                  onChange={(event) =>
                    setEditor({ ...editor, issuer: event.target.value })
                  }
                />
              </div>
              <div className="shell__row">
                ...
                <input
                  ...
                  value={editor.keyId}
                  onChange={(event) =>
                    setEditor({ ...editor, keyId: event.target.value })
                  }
                  placeholder={t("shell.form.jwtKeyIdPlaceholder")}
                />
              </div>
              <div className="shell__row">
                <label htmlFor="privateKeyPem">
                  {t("shell.form.jwtPrivateKeyPem")}
                </label>
                <textarea
                  id="privateKeyPem"
                  value={editor.privateKeyPem}
                  onChange={(event) =>
                    setEditor({ ...editor, privateKeyPem: event.target.value })
                  }
                  rows={8}
                  placeholder={t("shell.form.jwtPrivateKeyPlaceholder")}
                />
              </div>
              <div className="shell__row">
