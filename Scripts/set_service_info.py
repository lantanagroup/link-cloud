...
def update_java_config(yaml_path, commit, product_version):
    with open(yaml_path, "r", encoding="utf-8") as f:
        try:
            data = yaml.safe_load(f) or {}
        except yaml.YAMLError:
            print(f"ERROR: Could not parse {yaml_path} as YAML")
            sys.exit(1)
...
def update_dotnet_config(json_path, commit, product_version):
    with open(json_path, "r", encoding="utf-8") as f:
        try:
            data = json.load(f)
        except json.JSONDecodeError:
            print(f"ERROR: Could not parse {json_path} as JSON")
            sys.exit(1)
...
def update_commit(directory, service_name, commit, product_version=None):
    ...
    if os.path.exists(json_path):
        update_dotnet_config(json_path, commit, product_version)
    elif os.path.exists(yaml_path):
        update_java_config(yaml_path, commit, product_version)
    else:
...
if __name__ == "__main__":
    ...
    if not all([directory, service_name, commit]):
        ...
        print("  python update_commit.py <Directory> <ServiceName> <Commit> <ProductVersion>")
        print("Or environment variables:")
        print("  BUILD_SOURCESDIRECTORY, SERVICENAME, GIT_COMMIT, PRODUCT_VERSION")
        sys.exit(1)

    update_commit(directory, service_name, commit, product_version)