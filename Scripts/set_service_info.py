...
def update_java_config(yaml_path, commit, product_version, test_mode=False):
...
def update_dotnet_config(json_path, commit, product_version, test_mode=False):
...
def update_commit(directory, service_name, commit, product_version=None, test_mode=False):
    ...
    if os.path.exists(json_path):
        update_dotnet_config(json_path, commit, product_version, test_mode)
    elif os.path.exists(yaml_path):
        update_java_config(yaml_path, commit, product_version, test_mode)
    else:
...
if __name__ == "__main__":
    ...
    product_version = get_param(sys.argv[1:], 3, 'PRODUCT_VERSION')
    test_mode = get_param(sys.argv[1:], 4, 'TEST_MODE') == 'true'

    if not all([directory, service_name, commit]):
        print("Error: Missing required parameters.")
        print("Provide either command line arguments:")
        print("  python update_commit.py <Directory> <ServiceName> <Commit> <ProductVersion> <TestMode?>")
        print("Or environment variables:")
        print("  BUILD_SOURCESDIRECTORY, SERVICENAME, GIT_COMMIT, PRODUCT_VERSION TEST_MODE")
        sys.exit(1)

    update_commit(directory, service_name, commit, product_version, test_mode)