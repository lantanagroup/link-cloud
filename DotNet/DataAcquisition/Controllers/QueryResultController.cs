...
public class FhirQueriesController : ControllerBase
{
        ...
        [FromRoute] string facilityId, [FromQuery] string correlationId, [FromQuery] string patientId, [FromQuery] string resourceType, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(new EventId(DataAcquisitionConstants.LoggingIds.GetItem, "Get FHIR Query Results"), ex, "An exception occurred while attempting to retrieve FHIR Query Results for correlationId {correlationId}", correlationId);
            throw;
        }
    }
}
