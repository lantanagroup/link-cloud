...
public class RetryListener : BackgroundService
{
    ...
    private async void StartConsumerLoop(CancellationToken cancellationToken)
    {
        ...
        try
        {
            consumer.Subscribe(new List<string>() { KafkaTopic.PatientIDsAcquiredRetry.GetStringValue() });

            _logger.LogInformation("Started Census Service Retry consumer for topics: [{1}] {2}", string.Join(", ", consumer.Subscription), DateTime.UtcNow);
                    ...
                    await consumer.ConsumeWithInstrumentation(async (result, cancellationToken) =>
                    {
                        ...
                        try
                        {
                            ...
                            if (consumeResult.Message.Headers.TryGetLastBytes(KafkaConstants.HeaderConstants.ExceptionService, out var exceptionService))
                            {
                                ...
                                if (Encoding.UTF8.GetString(exceptionService) != "Census")
                                {
                        ...
                        {
                            _logger.LogError(ex, "Error in Census Service Retry consumer for topics: [{1}] at {2}", string.Join(", ", consumer.Subscription), DateTime.UtcNow);
                        }
