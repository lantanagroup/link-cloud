    ...
    public class FacilityConfigurationService : IFacilityConfigurationService
    {
        ...
        private void ValidateFacility(FacilityConfigModel facility)
        {
            ...
            try
            {
                ...
                // verify the id of the time zone is IONA format
                if (!timeZoneInfo.HasIanaId)
                {
                    _logger.LogError("Incorrect Timezone format: " + facility.TimeZone +  "(Time zones should be in IONA format for example: America/Chicago)");
                    throw new ApplicationException("Incorrect Timezone format: " + facility.TimeZone + " (Time zones should be in IONA format for example: America/Chicago)");
                }
            }
