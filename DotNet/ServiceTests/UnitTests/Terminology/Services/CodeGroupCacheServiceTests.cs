...
456,Another Display";
    ...
    [Fact]
    public async Task ReplaceCodesFromCsv_MalformedStatus_ThrowsAndLeavesCacheIntact()
    {
        using var memoryCache = new MemoryCache(new MemoryCacheOptions());
        var service = BuildServiceWithLoadedGroups(memoryCache);
        await service.LoadCache();
        ...
        var before = service.GetCodeGroupById(CodeGroup.CodeGroupTypes.CodeSystem, "test-cs")!
            .Codes.Values.Sum(c => c.Count);

        // The second row's status is not a CodeStatus; CsvHelper throws mid-enumeration, which is
        // the case most likely to leave a half-written cache if the codes were applied in place.
        Assert.ThrowsAny<CsvHelperException>(() => service.ReplaceCodesFromCsv(
            CodeGroup.CodeGroupTypes.CodeSystem, "test-cs", null,
            "code,display,status\r\nAAA,Good,Active\r\nBBB,Bad,NotAStatus\r\n"));

        Assert.Equal(before, service.GetCodeGroupById(CodeGroup.CodeGroupTypes.CodeSystem, "test-cs")!
            .Codes.Values.Sum(c => c.Count));
    }
