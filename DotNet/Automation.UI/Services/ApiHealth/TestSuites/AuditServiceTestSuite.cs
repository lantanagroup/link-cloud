...
using StepNames = Automation.UI.Services.ApiHealth.TestSuites.ApiEndPointLibrary.AuditSteps;

namespace Automation.UI.Services.ApiHealth.TestSuites;

/// <summary>
/// Exercises Audit service health and information endpoints.
/// </summary>
public sealed class AuditServiceTestSuite : ServiceTestSuiteBase
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IOptions<ServiceRegistry> _serviceRegistry;

    public override string ServiceName => "Audit";

    public AuditServiceTestSuite(
        ...
        CancellationToken ct = default)
    {
        ...
        var baseUrl = _serviceRegistry.Value.AuditServiceUrl?.TrimEnd('/');

        if (string.IsNullOrWhiteSpace(baseUrl))
        {
            const string error =
                "ServiceRegistry:AuditServiceUrl is not configured.";
            ...
            })
            {
                results.Add(new ApiTestRunResult
                {
                    ...
                    ErrorMessage = error,
                    RequestBody =
                        "Request was not sent because the Audit service URL is missing.",
                    ResponseBody =
                        "Response was not received because the Audit service URL is missing.",
