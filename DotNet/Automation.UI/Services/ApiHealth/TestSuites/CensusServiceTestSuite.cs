...
public sealed class CensusServiceTestSuite : ServiceTestSuiteBase
{
    ...
    [
        ...
        Step("Config POST → 201", "Creates census config for valid facility", "/api/census/config"),
        Step("Config POST → 400 (empty ScheduledTrigger)", "Returns 400 when ScheduledTrigger is empty", "/api/census/config"),
        Step("Config POST → 400 (empty FacilityId)", "Returns 400 when FacilityId is empty", "/api/census/config"),
        Step("Config POST → 400 (invalid cron)", "Returns 400 when ScheduledTrigger is not a valid cron expression", "/api/census/config"),
    ...
    public override async Task<IReadOnlyList<ApiTestRunResult>> ExecuteAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Config POST → 201
            results.Add(await RunStepAsync("Config POST → 201", 201, async () =>
            {
            ...
            results.Add(await RunStepAsync("Config POST → 400 (empty ScheduledTrigger)", 400, async () =>
            ...
            results.Add(await RunStepAsync("Config POST → 400 (empty FacilityId)", 400, async () =>
                await _client.CreateCensusConfigAsync(new CensusConfigApiModel
                {
                    FacilityId = "",
                    ScheduledTrigger = "0 0/5 * * * ?",
            ...
            results.Add(await RunStepAsync("Config POST → 400 (invalid cron)", 400, async () =>
                await _client.CreateCensusConfigAsync(new CensusConfigApiModel
                {
                    FacilityId = facilityId,
            ...
            // Config GET → 200
            results.Add(await RunStepAsync("Config GET → 200", 200, async () =>
                await _client.GetCensusConfigAsync(facilityId, ct), ct: ct));

            // --- PUT /api/census/config/{facilityId} ---

            // Config PUT → 202
            results.Add(await RunStepAsync("Config PUT → 202", 202, async () =>
            ...
            results.Add(await RunStepAsync("Config PUT → 400 (mismatched FacilityId)", 400, async () =>
                await _client.UpdateCensusConfigAsync(facilityId, new CensusConfigApiModel
                {
                    FacilityId = "wrong-id",
                    ScheduledTrigger = "0 0/10 * * * ?",
            ...
            results.Add(await RunStepAsync("Config PUT → 400 (empty FacilityId)", 400, async () =>
                await _client.UpdateCensusConfigAsync(facilityId, new CensusConfigApiModel
                {
                    FacilityId = "",
                    ScheduledTrigger = "0 0/10 * * * ?",
            ...
            results.Add(await RunStepAsync("Config PUT → 400 (empty ScheduledTrigger)", 400, async () =>
                await _client.UpdateCensusConfigAsync(facilityId, new CensusConfigApiModel
                {
                    FacilityId = facilityId,
                    ScheduledTrigger = "",
            ...
            results.Add(await RunStepAsync("Config PUT → 400 (invalid cron)", 400, async () =>
                await _client.UpdateCensusConfigAsync(facilityId, new CensusConfigApiModel
                {
                    FacilityId = facilityId,
                    ScheduledTrigger = "not-a-cron",
            ...
            results.Add(await RunStepAsync("DisableJobs DELETE → 204", 204, async () =>
            ...
            results.Add(await RunStepAsync("DisableJobs DELETE → 400", 400, async () =>
                await _client.DisableFacilityJobsAsync(" ", ct), ct: ct));

            // --- PATCH /api/census/config/{facilityId}/jobs/restore ---

            // EnableJobs PATCH → 204 (re-enables jobs for the same facility)
            results.Add(await RunStepAsync("EnableJobs PATCH → 204", 204, async () =>
            ...
            results.Add(await RunStepAsync("EnableJobs PATCH → 400", 400, async () =>
            ...
            results.Add(await RunStepAsync("HistoricalEncounters GET → 400", 400, async () =>
            ...
            results.Add(await RunStepAsync("PatientEvents GET → 404", 404, async () =>
                await _client.GetPatientEventsAsync(facilityId, ct), ct: ct));

            // PatientEvents GET → 400 (empty facilityId)
            results.Add(await RunStepAsync("PatientEvents GET → 400", 400, async () =>
            ...
            results.Add(await RunStepAsync("PatientEvent DELETE → 400 (empty id)", 400, async () =>
                await _client.DeletePatientEventAsync(" ", ct), ct: ct));

            // PatientEvent DELETE → 404 (random id never in DB)
            results.Add(await RunStepAsync("PatientEvent DELETE → 404 (non-existent)", 404, async () =>
            ...
            results.Add(await RunStepAsync("PatientEventByCorrelation DELETE → 400", 400, async () =>
            ...
            results.Add(await RunStepAsync("Config GET → 404", 404, async () =>
                await _client.GetCensusConfigAsync(facilityId, ct), ct: ct));
        }
        finally
        {
