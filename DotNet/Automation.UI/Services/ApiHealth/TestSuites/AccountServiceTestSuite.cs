...
using StepNames = Automation.UI.Services.ApiHealth.TestSuites.ApiEndPointLibrary.AccountSteps;

namespace Automation.UI.Services.ApiHealth.TestSuites;

/// <summary>
/// Exercises Account service health and information endpoints.
/// </summary>
public sealed class AccountServiceTestSuite : ServiceTestSuiteBase
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IOptions<ServiceRegistry> _serviceRegistry;

    public override string ServiceName => "Account";

    public AccountServiceTestSuite(
        ...
        CancellationToken ct = default)
    {
        ...
        var baseUrl = _serviceRegistry.Value.AccountServiceUrl?.TrimEnd('/');

        if (string.IsNullOrWhiteSpace(baseUrl))
        {
            const string error =
                "ServiceRegistry:AccountServiceUrl is not configured.";
            ...
            })
            {
                results.Add(new ApiTestRunResult
                {
                    ...
                    ErrorMessage = error,
                    RequestBody =
                        "Request was not sent because the Account service URL is missing.",
                    ResponseBody =
                        "Response was not received because the Account service URL is missing.",
