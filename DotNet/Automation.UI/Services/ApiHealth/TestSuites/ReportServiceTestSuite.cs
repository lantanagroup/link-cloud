...
public sealed class ReportServiceTestSuite : ServiceTestSuiteBase
{
    ...
    [
        ...
        Step("GET /api/schedules/{id}", "Get Schedule → 200 (has data)", "Returns seeded schedule by id"),
        Step("GET /api/schedules/{id}", "Get Schedule → 404", "Returns 404 for non-existent schedule"),
        Step("GET /api/schedules/{id}", "Get Schedule → 400 (bad guid)", "Returns 400 for invalid GUID format"),
    ...
    public override async Task<IReadOnlyList<ApiTestRunResult>> ExecuteAsync(CancellationToken ct = default)
    {
        ...
        // === GET /api/schedules/{id} ===
        results.Add(await RunStepAsync("Get Schedule → 404", 404, async () =>
            await _client.GetScheduleAsync(fakeReportId, ct), ct: ct));

        results.Add(await RunStepAsync("Get Schedule → 400 (bad guid)", 400, async () =>
            await _client.GetScheduleAsync("not-a-valid-guid", ct), ct: ct));

        // === GET /api/schedules/facilities/{id} ===
        results.Add(await RunStepAsync("Get By Facility → 400 (empty)", 400, async () =>
            await _client.GetSchedulesByFacilityAsync(" ", cancellationToken: ct), ct: ct));

        results.Add(await RunStepAsync("Get By Facility → 200 (empty)", 200, async () =>
        ...
        // === DELETE /api/schedules/{id} ===
        results.Add(await RunStepAsync("SoftDelete → 400 (bad guid)", 400, async () =>
            await _client.SoftDeleteScheduleAsync("not-a-valid-guid", ct), ct: ct));

        results.Add(await RunStepAsync("SoftDelete → 404", 404, async () =>
            await _client.SoftDeleteScheduleAsync(fakeReportId, ct), ct: ct));

        // === PATCH /api/schedules/{id}/restore ===
        results.Add(await RunStepAsync("Restore → 400 (bad guid)", 400, async () =>
            await _client.RestoreScheduleAsync("not-a-valid-guid", ct), ct: ct));

        results.Add(await RunStepAsync("Restore → 404", 404, async () =>
            await _client.RestoreScheduleAsync(fakeReportId, ct), ct: ct));

        // === PATCH /api/schedules/facility/{id}/status ===
        results.Add(await RunStepAsync("SetStatus → 204", 204, async () =>
        ...
        results.Add(await RunStepAsync("Entry Count → 400 (bad guid)", 400, async () =>
        ...
        results.Add(await RunStepAsync("Entry Summary → 400 (bad guid)", 400, async () =>
        ...
        results.Add(await RunStepAsync("Entry By Patient → 400 (bad guid)", 400, async () =>
            await _client.GetEntryByScheduleAndPatientAsync("not-a-valid-guid", fakePatientId, ct), ct: ct));

        await AddSeededOrSkipAsync(
            seededPatientId != null,
            "Entries By Patient → 200 (has data)",
            200,
            () => _client.GetEntriesByPatientAsync(seededPatientId!, ct),
            seededDataUnavailableReason);

        results.Add(await RunStepAsync("Entries By Patient → 404", 404, async () =>
            await _client.GetEntriesByPatientAsync(fakePatientId, ct), ct: ct));

        // === GET /api/resources/search ===
        results.Add(SkipStepAsync("Resource GET → 200 (has data)", "Resource rows are intentionally not persisted for seeded runs in this environment, so no deterministic resource id exists."));

        results.Add(await RunStepAsync("Resource GET → 404", 404, async () =>
            await _client.GetResourceByIdAsync(Guid.NewGuid().ToString(), ct), ct: ct));

        results.Add(await RunStepAsync("Resource GET → 400 (bad guid)", 400, async () =>
            await _client.GetResourceByIdAsync("not-a-valid-guid", ct), ct: ct));

        await AddSeededOrSkipAsync(
            scheduleId != null,
            "Resources By Schedule → 200 (has data)",
            200,
            () => _client.GetResourcesByScheduleAsync(scheduleId!, ct),
            scheduleUnavailableReason);

        results.Add(await RunStepAsync("Resources By Schedule → 400 (bad guid)", 400, async () =>
            await _client.GetResourcesByScheduleAsync("not-a-valid-guid", ct), ct: ct));

        await AddSeededOrSkipAsync(
            scheduleId != null && seededPatientId != null,
            "Resources By Schedule+Patient → 200 (has data)",
            200,
            () => _client.GetResourcesByScheduleAndPatientAsync(scheduleId!, seededPatientId!, ct),
            seededDataUnavailableReason);

        results.Add(await RunStepAsync("Resources By Schedule+Patient → 400 (bad guid)", 400, async () =>
            await _client.GetResourcesByScheduleAndPatientAsync("not-a-valid-guid", fakePatientId, ct), ct: ct));

        await AddSeededOrSkipAsync(
            seededPatientId != null,
            "Resources By Patient → 200 (has data)",
            200,
        ...
        results.Add(await RunStepAsync("InitPop Count → 400 (empty id)", 400, async () =>
            await _client.GetInitialPopulationCountAsync(" ", ct), ct: ct));

        results.Add(await RunStepAsync("InitPop Count → 400 (bad guid)", 400, async () =>
