...
public sealed class DataAcquisitionTestSuite : ServiceTestSuiteBase
{
    ...
    [
        // POST /api/data/fhirQueryConfiguration
        Step("FhirConfig POST → 201", "Creates FHIR query config", "/api/data/fhirQueryConfiguration"),
        Step("FhirConfig POST → 409", "Returns 409 when config already exists", "/api/data/fhirQueryConfiguration"),
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(seededFacilityId) && !string.IsNullOrWhiteSpace(seededReportId))
            {
                ...
                await AddSeededOrSkipTypedAsync(
                    hasSeededSearch,
                    "AcquisitionLogs GET → 200 (has data)",
                    200,
            ...
            {
                results.Add(SkipStepAsync("AcquisitionLogs GET → 200 (has data)", seedUnavailable));
                results.Add(SkipStepAsync("StatusCounts GET → 200 (has data)", seedUnavailable));
                results.Add(SkipStepAsync("Summary GET → 200 (has data)", seedUnavailable));
                results.Add(SkipStepAsync("ResourceIds GET → 200 (has data)", seedUnavailable));
            }
            ...
            results.Add(await RunStepAsync("FhirConfig POST → 201", 201, async () =>
            {
            ...
            results.Add(await RunStepAsync("QueryPlan GET → 404", 404, async () =>
                await _client.GetQueryPlanAsync(facilityId, "Discharge", ct), ct: ct));

            // DELETE FHIR Query Config
            results.Add(await RunStepAsync("FhirConfig DELETE → 202", 202, async () =>
            {
            ...
            results.Add(await RunStepAsync("FhirConfig GET → 404", 404, async () =>
                await _client.GetFhirQueryConfigurationAsync(facilityId, ct), ct: ct));

            // --- Read-only operations (non-existent resources � prove reachability) ---
            var fakeReportId = Guid.NewGuid().ToString();

            results.Add(await RunStepAsync("AcquisitionLogs GET → 200", 200, async () =>
