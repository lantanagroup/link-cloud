...
public abstract class LinkApiClientBase : IDisposable
{
    ...
    protected static async Task<LinkApiResponse<T>> SendAsync<T>(Func<Task<IFlurlResponse>> action)
    {
        try
        {
            ...
            if (statusCode is >= 200 and < 300)
            {
                ...
                return new LinkApiResponse<T> { StatusCode = statusCode, Body = body, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
            }
            var raw = await response.GetStringAsync();
            return new LinkApiResponse<T> { StatusCode = statusCode, RawBody = raw, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
        catch (FlurlHttpException ex)
        {
            ...
            return new LinkApiResponse<T> { StatusCode = ex.StatusCode ?? 0, RawBody = raw, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
    }
    ...
    protected static async Task<LinkApiResponse<string>> SendStringAsync(Func<Task<IFlurlResponse>> action)
    {
        try
        {
            ...
            if (statusCode is >= 200 and < 300)
                return new LinkApiResponse<string> { StatusCode = statusCode, Body = body, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
            return new LinkApiResponse<string> { StatusCode = statusCode, RawBody = body, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
        catch (FlurlHttpException ex)
        {
            var raw = await ex.GetResponseStringAsync();
            ...
            var requestMethod = ex.Call?.HttpRequestMessage?.Method.Method;
            var traceId = ExtractTraceId(ex.Call?.Response);
            return new LinkApiResponse<string> { StatusCode = ex.StatusCode ?? 0, RawBody = raw, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
    }
    ...
    protected static async Task<LinkApiResponse> SendAsync(Func<Task<IFlurlResponse>> action)
    {
        try
        {
            ...
            var requestMethod = response.ResponseMessage.RequestMessage?.Method.Method;
            var traceId = ExtractTraceId(response);
            return new LinkApiResponse { StatusCode = response.StatusCode, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
        catch (FlurlHttpException ex)
        {
            var raw = await ex.GetResponseStringAsync();
            ...
            var requestMethod = ex.Call?.HttpRequestMessage?.Method.Method;
            var traceId = ExtractTraceId(ex.Call?.Response);
            return new LinkApiResponse { StatusCode = ex.StatusCode ?? 0, RawBody = raw, RequestUrl = requestUrl, RequestMethod = requestMethod, TraceId = traceId };
        }
    }
