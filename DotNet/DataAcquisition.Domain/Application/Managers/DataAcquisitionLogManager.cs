...
using DataAcquisition.Domain.Infrastructure.Models.Exceptions;
...
public class DataAcquisitionLogManager : IDataAcquisitionLogManager
{
    ...
    {
        var resultSet = await _database.DataAcquisitionLogRepository.FindAsync(x => x.FacilityId == facilityId && x.CorrelationId == correlationId && x.ScheduledReport.ReportTrackingId == reportTrackingId, cancellationToken);
        resultSet.ForEach(x => x.TailSent = true);
        resultSet.ForEach(async x => await _database.DataAcquisitionLogRepository.UpdateAsync(x, cancellationToken));
    }
}
