...
- `suppressMessageIds`: optional list of stable HAPI message ID strings (from
  `org.hl7.fhir.utilities.i18n.I18nConstants`) that this rule should drop at the
  `isSuppressMessageId(path, messageId)` hook. SUPPRESS strategy — message is
  generated by the validator but never reaches the `OperationOutcome`. Independent
  of `scope` and orthogonal to `strategy`: a single rule can carry both a `scope`
  (for SKIP) and `suppressMessageIds` (for SUPPRESS) — `unknown_code_system` is
  the canonical example, using `scope.codeSystems` for URL-specific matcher
  branches and `suppressMessageIds` for the generic-shape branches. Phase 4.
- `suppressPathPatterns`: optional regex patterns matched against the `path`
  argument HAPI passes to `isSuppressMessageId`. **Narrows where** a rule's
  `suppressMessageIds` fire. When null or empty, the rule fires on any path that
  produces one of its message IDs (`unknown_code_system`'s shape — globally
  acceptable message families). When non-empty, the rule fires only when at
  least one pattern matches the path (`medicationrequest_requester...`'s shape —
  the team's stance is path-specific). Phase 4.

  Without `suppressPathPatterns` available, the only way to encode a
  path-specific SUPPRESS would be unscoped suppression at the message-ID layer,
  which silences the message everywhere — the same over-suppress hazard as
  unscoped SKIP without `excludeActions`. Path narrowing is the precise
  equivalent of `excludeActions` for SUPPRESS: it converts "I want to silence
  this specific case" from "kill the whole HAPI message family" into "kill it
  only at these elements."
...
### Phase 4 — Add isSuppressMessageId support and migrate ✔ DONE

This phase shipped the SUPPRESS strategy end-to-end, with a mid-phase
mechanism extension (`suppressPathPatterns` for path narrowing) and three
rule migrations. `unknown_code_system`'s migration is now functionally
complete (7 of 8 matcher branches handled by advisor hooks); the eighth
branch — `V3 ValueSet ActEncounterCode` — is deliberately left as the LABEL
fallback because it needs a `suppressValueSets` narrowing axis we deferred.

**Shipped:**

- `Category.suppressMessageIds: List<String>` — JSON-array-of-strings column,
  null-safe via `SuppressMessageIdsConverter`. Stable HAPI message IDs (from
  `org.hl7.fhir.utilities.i18n.I18nConstants`) that the advisor drops at
  `isSuppressMessageId(...)`. Independent of `strategy` and `scope`; a rule
  can carry both SKIP and SUPPRESS axes simultaneously.
- `Category.suppressPathPatterns: List<String>` — sibling field, reuses the
  same JSON converter. Regex patterns matched against the `path` argument
  HAPI passes to `isSuppressMessageId`. Empty / null → "fire on any path"
  (the default that `unknown_code_system` relies on); non-empty → narrow.
- `CategoryBackedPolicyAdvisor.isSuppressMessageId(path, messageId)` override.
  Suppress storage is `Map<String /* messageId */, List<CompiledSuppressRule>>`:
  O(1) lookup on messageId, then linear scan of candidates checking
  `pathPatterns`. First-matching-rule wins counter credit; multiple rules can
  register the same message ID with different path narrowings (narrow + broad
  fallback is the canonical composition).
- Load-time validation: `acceptable=false` SUPPRESS rules are demoted with
  WARN (same pattern as the SKIP path). A rule with `suppressPathPatterns`
  whose every regex fails to compile is registered for audit visibility but
  never matches — falling back to "fire on any path" would silently widen
  scope to global suppression the author didn't request.
- Three rules migrated. See Migration log below for each shape.

**Schema decision landed:** `suppressMessageIds` is a sibling field on
`Category`, not a member of `scope`. SKIP and SUPPRESS are different
strategies operating at different lifecycle stages; keeping them as parallel
top-level fields keeps the rule shape easier to read. A single rule with
`strategy: SKIP` and both `scope` AND `suppressMessageIds` is valid and
exercised in production by `unknown_code_system`.

**Carry-out items for Phase 5 or follow-up:**

- `unknown_code_system` matcher branch 8 (`V3 ValueSet ActEncounterCode`)
  still on LABEL fallback. Needs a `suppressValueSets` narrowing axis or a
  per-code narrowing — neither was in this PR's scope. Documented in the
  rule's `guidance`.
- `non_loinc_code_glucose_point_of_testing` not migrated. Its matcher
  narrows by ValueSet URL (`observation-codes`) AND specific code substrings
  (`GLUMT`, `270`). Needs `suppressValueSets` plus likely per-code narrowing
  to migrate safely. Stays LABEL.
- Several other `acceptable: true` rules may benefit from SUPPRESS migration
  after a representative-bundle audit identifies which `I18nConstants` IDs
  they target. The mechanism is in place; further migrations are data-only
  changes after the I18nConstants mapping per rule is verified.

**Tests added in Phase 4:** 17 new advisor tests covering SUPPRESS load,
hook fire/no-fire, path-narrowing matrix (empty / matching / non-matching /
multi-pattern / partial-failure / all-invalid), and the
`scope`+`suppressMessageIds` composability case. Total validation tests:
98 (up from 81 at the start of Phase 4).
...
| 2026-06-16 | `unknown_code_system` (Phase 4 extension) | `SKIP` + `SUPPRESS` | 7 of 8 matcher branches | Phase 4 adds `suppressMessageIds` with seven I18nConstants to cover three of the four generic-shape branches that Phase 1's `scope.codeSystems` couldn't reach: `Terminology_TX_System_Unknown` ('Unknown Code System X'), `Coding_has_no_system__cannot_validate` + `TERMINOLOGY_TX_SYSTEM_NO_CODE` ('A code with no system'), and the `UNKNOWN_CODESYSTEM*` family (`UNKNOWN_CODESYSTEM`, `UNKNOWN_CODESYSTEM_VERSION`, `UNKNOWN_CODESYSTEM_VERSION_UNK`, `UNKNOWN_CODESYSTEM_VERSION_NONE` — the modern HAPI shape of the rule's stale `^CodeSystem is unknown` matcher branch). Branch 8 (`V3 ValueSet ActEncounterCode`) still on LABEL — requires a `suppressValueSets` axis we deferred. After this commit the rule fires as either `outcome=skipped` (URL-specific branches) or `outcome=suppressed` (the seven message IDs); `outcome=labeled` should drop close to zero except on cold start. |
| 2026-06-23 | `medicationrequest_requester_does_not_have_a_proper_reference` | `SUPPRESS` (path-narrowed) | Complete | First use of the Phase 4 `suppressPathPatterns` mechanism. `suppressMessageIds: ["Reference_REF_NoDisplay"]` (HAPI template exactly matches the rule's MESSAGE matcher branch) narrowed by `suppressPathPatterns: ["ofType\\(MedicationRequest\\).*\\.requester"]` (mirrors the rule's EXPRESSION matcher branch in a more lenient form). Path narrowing is the load-bearing safety property — without it the rule would suppress every "Reference should have a display" warning across every Reference element in every resource, which the team has not classified as acceptable. Matcher kept as defensive fallback for cold start. |
| 2026-06-23 | `unresolved_code_system` | `SUPPRESS` (path-narrowed) | Complete | `suppressMessageIds: ["Type_Specific_Checks_DT_URL_Resolve"]` (the modern HAPI message for "No definition could be found for URL value X" — semantically equivalent to the rule's stale matcher text `URL value X does not resolve`) narrowed by `suppressPathPatterns: ["\\.coding\\[[0-9]+\\]\\.system"]` (exact mirror of the rule's EXPRESSION matcher branch). Path narrowing keeps the suppression scoped to code-system URL resolution failures — canonical URL resolution (profiles, ValueSets) continues to validate normally. Matcher kept as defensive fallback. |
...
| `src/main/java/.../converters/SuppressMessageIdsConverter.java` | JPA `AttributeConverter` for `List<String>` ↔ JSON column. Reused for `suppressMessageIds` AND `suppressPathPatterns` since the storage shape is identical. Added in Phase 4 |
...
| `src/main/java/.../services/CategoryBackedPolicyAdvisor.java` | HAPI policy advisor — runs `SKIP` decisions at `policyForCodedContent` (Phase 1, Phase 2 extension for `excludeActions`) and SUPPRESS decisions at `isSuppressMessageId` (Phase 4, Phase 4 extension for `suppressPathPatterns`). |
