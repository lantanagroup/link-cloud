...
///
/// Per-patient stages (each worth 1 unit):
///   1. Data Acquired -- DataAcquisitionLog status = Completed for this patient
///   2. Measure Evaluated -- EntryMeasureReport exists with a MeasureReportId
///   3. Validation Complete -- ReportEntry.ReportingStatus >= PassedValidation or FailedValidation
///   4. Patient Submitted -- ReportEntry.SubmissionStatus = Submitted
///
/// Report-level milestones (1 unit each):
///   5. Schedule Created -- ReportSchedule exists in the database
///   6. Report Finalized -- ScheduleStatus = Submitted
///
/// Total units = (expectedPatients x 4) + 2.
/// </summary>
public class PipelineProgressTracker
{
    ...
    public PipelineProgressTracker(ITestOutputHelper output, int expectedPatientCount)
    {
        _output = output;
    ...
    public async Task UpdateAsync(string facilityId, string reportId)
    {
        try
        {
            ...
            await using var reportDb = DatabaseConnectionFactory.CreateReportDbContext();
            ...
            await using var acqDb = DatabaseConnectionFactory.CreateDataAcquisitionDbContext();
